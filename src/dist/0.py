...
face_bug = None
# 紧急消息序号
emergence_number = randint(10, 100)
...
def handler_receive_msg(msg, isGroupChat=False):
    ...
    if msg['Type'] == 'Text' \
        ...
        if auto_reply:
            if msg_content.rfind(str(emergence_number)) >= 0:
                print(msg_time_rec + "紧急消息," + msg_from + ":" + msg_content)
                itchat.send("已收到，我将尽快处理", msg['FromUserName'])
                # MessageBox(0,msg_time_rec + "紧急消息," + msg_from + ":" + msg_content,'重要消息',MB_OK)
            else:
                reply_data = {
                    "reqType": 0,
                    "perception": {
                ...
                reply = "以下为自动回复\n" \
                        ...
                        + "\n紧急情况请在消息开头添加数字" \
                        + str(emergence_number) \
                        + "\n" + '例如' + '"' + str(emergence_number) + '我要请你吃饭！"'
