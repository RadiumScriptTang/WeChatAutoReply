...
def handler_receive_msg(msg, isGroupChat=False):
    ...
    if msg['Type'] == 'Text' \
        ...
        elif messageTransferFriends == msg["FromUserName"] and transferFriends is not None:
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: wechatAutoReply default_reply_file [transfer_friend_remark_name]")
        print("wechatAutoReply error: no default file found!")
    if len(sys.argv) >= 2:
        f_reply = open(sys.argv[1],"r",encoding="UTF-8")
        firstReplyMessage = "".join(f_reply)
    if len(sys.argv) == 3:
        transferFriends = sys.argv[2]
    else:
        transferFriends = None
    if len(sys.argv) > 1:
